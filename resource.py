...
class Resource(object):
    ...
    def edit_resource(self, request):
        ...
        '''
        self.payload = (request.content_type, requst.payload)
        payload_parser = request.payload[:]
        payload_parser = payload_parser.split(",")
        unix = payload_parser[0]
        client_ID = payload_parser[1]
        client_counter = payload_parser[2]
        date = str(datetime.datetime.fromtimestamp(unix).strftime('%Y-%m-%d %H:%M:%S'))
        c.execute("INSERT INTO iot_table (unix, datestamp, client_ID, client_counter) VALUES (?,?,?,?)",(unix,date,client_ID,client_counter))
        conn.commit() 
